...
class leaveclass extends general {
	...
	#insert values for leave trasanction
	...
	#insert into leave transaction with remarks 
	public function insertLTS($appno, $staffid, $role, $transactionid, $timeviewed, $reco, $sdate, $edate, $remarks = null){
	...
	#insert resumption details into transaction table
	...
	#update details of staff to show staff has filled the resumption form
	...
	#update the resumption date of staff to show the date of resumption
	...
	#update staff details in approvedleaves table to show release date
	...
	#update approvedleaves table to show released start and end date
	...
	#initial insertion into approvedleaves table
	...
	#update remarks and recommendation in the leaveapplication table
	...
	#insert resumption details of staff 
	...
	public function insertLeaveScheduleTransaction($transactionDate, $transaactionNo, $cursession, $officer, $recc, $comment, $action){
		#this function inserts transaction details for logging purposes
	...
	public function insertLeaveSchedule($schedule, $cursession ){
		#function inserts the details of leaveschedule for staffs
	...
	public function insertNote($transactionDate, $transaactionNo, $cursession, $officer, $recc, $comment, $action){
		#this function inserts officers notes into the database
			$qry = "INSERT INTO leavescheduletransaction (transactionDate, transactionNo, session, officer, recommendation, comment, action) VALUE (:transactionDate, :transactionNo, :session, :officer, :recommendation, :comment, :action)";
	...
	public function getStatus($id, $cat){
		#staff views the leave status of themselves
	...
	public function getDeanView($deanid, $kol, $cat){
		#Dean views the leave status of staff in the college or directorate
		$query = "SELECT lt.timeviewed, l.staffid, lt.appno, lt.tstaffid, l.leavetype, l.reason, lt.recstartdate AS startdate , lt.recenddate AS enddate, l.location, lt.remarks, lt.status, st.coldirid, st.hod, st.dean, st.dept
          FROM leavetransaction AS lt
          INNER JOIN leaveapplication AS l
          ON lt.appno = l.appno
          INNER JOIN stafflst AS st
	...
	public function getHrView($hro){
		#HR views the leave status of staff 
		$query = "
				  SELECT lt.timeviewed, l.staffid, lt.appno, lt.tstaffid, l.leavetype, l.reason, lt.recstartdate AS startdate , lt.recenddate AS enddate, l.location, lt.remarks, lt.status, st.coldirid, st.hod, st.dean, st.dept
		          FROM leavetransaction AS lt
		          INNER JOIN leaveapplication AS l
		          ON lt.appno = l.appno
		          INNER JOIN stafflst AS st
	...
	public function getRegView($rego){
		#Registrar views the leave status of staffs
		$query = "SELECT lt.timeviewed, l.staffid, lt.appno, lt.tstaffid, l.leavetype, l.reason, lt.recstartdate AS startdate , lt.recenddate AS enddate, l.location, lt.remarks, lt.status, st.coldirid, st.hod, st.dean, st.dept
          FROM leavetransaction AS lt
          INNER JOIN leaveapplication AS l
          ON lt.appno = l.appno
	...
	public function getVcView($vco){
		...
		#VC views the leave status of staffs
	...
	public function leaveDetails($appno){
		##this function gets leave details of staff
		$query = "
	...
	public function leaveProgress($appno){
		#this function checks the leave progress of a staff via the appno
		$trqry = "SELECT *
	...
	public function leaveHistory($appno){
		#this function gets the leave history of a staff
	...
	public function checkSupervisor1($appno, $staffid){
		#this function checks if the supervisor has made a comment on the application before.
	...
	public function getApprovedLeaves(){
		#this function gets approved leaves from database
		$query ="
	...
	public function getRecHr(){
		#Reccommendation grouping for HR select drop down
	...
	public function staffDetailsforHR(){
		#this function gets the resumption details of staff
	...
	public function getOverstayedStaff(){
		#this function gets details of staff that have overstayed their leave.
		$query ="SELECT st.fname, st.sname, st.dept, al.staffid, al.appno, al.leavetype, al.apstartdate, al.apenddate, al.location, al.phone, al.releaseddate, al.resumeddate
                    FROM approvedleaves AS al
                    INNER JOIN stafflst AS st
                    ON st.staffid = al.staffid       
	...
	public function getDetailsByCategory($appno, $cat){
		...
		#this function gets the leave details of staff through the leave application number
		$query = "	SELECT *
          			FROM stafflst AS s
          			INNER JOIN leaveapplication AS l
	                  ON s.staffid = l.staffid
	...
	public function getResumptionViewHOD($dept, $hodid, $cat){
		...
		#this function gets the view for staffs who have resumed
	...
	public function getStaffRelease($staffid){
		#get the release details of staff
	...
	public function getDashboardOfficer($cursession){
		#this function gets officer details to make comments and view the leave schedule 
		$qryreg = "
	...
	public function getSchedule($cursession){
		#this function gets the schedule of leave
		$query1 = "SELECT *
                      FROM leaveschedule
                      WHERE session = '$cursession'";
	...
	public function isExistSchedule($cursession){
		#this function checks to see if leave schedule is existing
	...
	public function getAnnualLeaveSchdule(){
		#this function creates the leaveschedule from the current staff list
	...
	public function isLeaveAppExist($staffid, $leavetype){
		#this function checks if a staff has a leave application in progress
	...
	public function leaveReport($cursession){
		#this function gets details of leaves
		$query = "SELECT *
          FROM leaveschedule
          WHERE session = '$cursession'";
	...
	public function leaveReportHR($cursession){
		#this function gets the comment of the HR
		$hrqry = "
	...
	public function leaveReportReg($cursession){
		#this function gets the comment of the Registrar
	...
	public function leaveReportDFS($cursession){
		#this function gets the comment of the DFS
	...
	public function leaveReportVC($cursession){
		#this function gets the comment of the VC
		$qryvc = "
                  SELECT * FROM leavescheduletransaction
                  WHERE session = '$cursession' 
                ";
	...
	public function leaveReportIsVC($cursession){
		#this function is used to know if the person logged in is VC or not
...
{
	#this function is used to calculate the day which a staff will resume so as to know if the staff has overstayed their leave
...
{
	#this function calculates leave days allowed.
...
{
  #used in the annual leave report to know the total casual leave days gone by a staff in the session
            	$hquery = "SELECT ap.apstartdate, ap.apenddate 
		                   FROM approvedleaves AS ap
		                   WHERE ap.staffid LIKE '$staffid'
...
{
 #used in the annual leave report to know the total leave days gone by a staff in the session
           	$hquery = "SELECT ap.apstartdate, ap.apenddate 
		                   FROM approvedleaves AS ap
		                   WHERE ap.staffid LIKE '$staffid'
...
{
	#gets name of staff through the staffid
...
public function serAppno(){
	...
	public function trackid($appno){
		#selects the last transaction id from the transaction table
	...
	{
		//calculate number of days staff is using for leave
